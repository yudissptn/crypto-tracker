...
const CryptoList = async () => {
  ...
  return (
    <div className="flex flex-wrap gap-2 mx-auto max-w-screen-xl p-3">
      {data &&
        data.map((e: LatestListings) => {
          const price = (+e.quote["USD"].price.toPrecision(3)).toLocaleString(
            "en-US"
          );
          return (
            <Link href={`/crypto/${e.slug}`} key={e.id}>
              {" "}
              <Card
                className={`min-w-60 border-l-4 ${
                  e.quote["USD"].percent_change_24h >= 0
                    ? "border-cyan-400"
                    : "border-rose-500"
                }`}
              >
